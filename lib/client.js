...
Client.prototype.beginUpload = function() {
  ...
  var endpoint = util.format('%s://%s:%s/%s%s',
    self.options.beginProtocol,
    self.options.host,
    self.options.beginPort,
    self.options.prefix,
...
Client.prototype.upload = function(file) {
  ...
  var uploadChunk = function() {
    ...
    fs.read(fileDescriptor, buf, 0, self.options.chunkSize, this.bytesSent, function(err, bytesRead, buffer) {
      ...
      var endpoint = util.format('%s://%s:%s/%s%s?token=%s&offset=%s&length=%s&complete=%s',
        self.options.uploadProtocol,
        self.options.host,
        self.options.uploadPort,
        self.options.prefix,
